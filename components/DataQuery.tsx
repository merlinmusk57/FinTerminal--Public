...
export const DataQuery: React.FC = () => {
  ...
  const [startYear, setStartYear] = useState<number>(2025); // Default Updated
  ...
  const getExchangeRate = (targetCurrency: string) => {
      switch (targetCurrency) {
          case 'USD': return 0.128; // 1 HKD = 0.128 USD
          case 'RMB': return 0.92;  // 1 HKD = 0.92 RMB
          default: return 1;
      }
  };
  ...
  const filteredData = useMemo(() => {
    return financialData.filter(d => {
      ...
      // Segment Filter - CRITICAL for consistency with Peer Analysis
  ...
  const pivotTableData = useMemo(() => {
    ...
    filteredData.forEach(d => {
      ...
      if (selectedMetrics.includes(d.metric)) {
        ...
        // Only convert currency if the unit is NOT a percentage
        if (d.unit !== '%') {
             displayValue = d.value * rate;
        }
        ...
        if (d.unit === '%') {
            ...
            formattedValue = `${currencySymbol}${displayValue.toLocaleString(undefined, { maximumFractionDigits: 1 })}`;
        }
  ...
  }, [filteredData, selectedMetrics, selectedCurrency]);
  ...
  const handleExport = () => {
    ...
    // Construct an HTML Table which Excel can open perfectly
    // This is the standard way to generate .xls files client-side without heavy libraries
    let tableHTML = `
    ...
    pivotTableData.forEach(row => {
      ...
      selectedMetrics.forEach(m => {
        ...
        // Strip symbols for raw value in cell if needed, but simpler to just put the formatted string
